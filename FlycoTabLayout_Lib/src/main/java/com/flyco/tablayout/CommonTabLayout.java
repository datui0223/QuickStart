...
import com.flyco.tablayout.utils.StringUtil;
...
public class CommonTabLayout extends FrameLayout implements ValueAnimator.AnimatorUpdateListener {
    ...
    private void addTab(final int position, View tabView) {
        ...
        if(StringUtil.isValue(mTabEntitys.get(position).getTabUnselectedIconUrl())){
            Glide.with(getContext()).load(mTabEntitys.get(position).getTabUnselectedIconUrl()).into(iv_tab_icon);
        }else {
...
//            tv_tab_title.setPadding((int) mTabPadding, 0, (int) mTabPadding, 0);
            ...
            if (mIconVisible) {
                ...
                if(i == mCurrentTab){
                    if(StringUtil.isValue(tabEntity.getTabSelectedIconUrl())){
                        Glide.with(getContext()).load(tabEntity.getTabSelectedIconUrl()).into(iv_tab_icon);
                    }else {
                        iv_tab_icon.setImageResource(tabEntity.getTabSelectedIcon());
                    }
                }else {
                    if(StringUtil.isValue(tabEntity.getTabUnselectedIconUrl())){
                        Glide.with(getContext()).load(tabEntity.getTabUnselectedIconUrl()).into(iv_tab_icon);
                    }else {
